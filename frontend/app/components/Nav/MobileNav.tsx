...
export function MobileNav({
    ...
    const primaryItem = (item: SidebarNavItem) => {
        return <a
            key={item.href}
            href={item.href}
            className="relative flex flex-col items-center justify-center p-2 text-sm bg-yellow-500 hover:bg-yellow-400 rounded-full -mt-4 shadow-lg w-16 h-16"
        >
            {pathname === item.href && (
    ...
    return (
        <nav className="fixed bottom-0 left-0 right-0 bg-background border-t md:hidden">
            <div className="flex justify-around items-center h-14">
                {items.map((item) => (
                    item.isPrimary ? primaryItem(item) : (
                    <a
                        key={item.href}
                        href={item.href}
